export optSamplingBW2!, removeSample!
...
"""
function removeSamples!(m::Measurement{T}, cand::Vector{Int64}, tmp::Vector{T}, tmp_ifim::Matrix{T}, δj::Vector{T}, numSimSamp::Int64, batch::Int64) where T
  ...
  for i=1:numSimSamp-1
    ...
    removeSample!(m, cand2, δj2, idx, tmp, tmp_ifim)
    # re-estimate the changes in uncertainty
    getFIMChange!(δj2, m, cand2, tmp)
  end
...
"""
function getFIMChange!(δj::Vector{T}, m::Measurement{T}, cand::Vector{Int64}, tmp::Vector{T}) where T
  numComp = length(m.fim)
  shuffle!(cand)
  for k=1:numComp
...
function removeSample!(m::Measurement{T}, cand::Vector{Int64},δj::Vector{T},idx::Int64,tmp::Vector{T},tmp_ifim::Matrix{T}) where T
  numComp = length(m.fim)
